...
if(isset($_POST['param']))
{
    ...
    if($isResubmit) 
    {
        $meeting_query = "UPDATE mls_meeting SET date = '$date', location = '$location', commenced_time = '$ctime', adjourned_time = '$atime', meeting_minutes = '$minutes', attendees = '$attendees', confirmation_status = 'pending' WHERE meeting_id = ".$ml['meeting_id'].";";
        unset($_SESSION['ml']);
        session_commit();
    }
    else
    {
        $meeting_query = "INSERT INTO mls_meeting VALUES(NULL,'$date', '$location', '$ctime', '$atime', '$minutes', '$attendees', 'pending', '', 0, 'candidate', '');";
    }
    ...
    if ($meeting_result)
    {
        ...
        header('Location:student_index.php?cont=MLS&act=submit&page=confirm');
    }
    session_commit();
}
?>
<div class="container2">
  <div class="title">Create a new Meeting Log</div>
  <div class="content">
  <?php
